DROP PROCEDURE IF EXISTS IonSF.UpdateMarchMadnessR3Table;
SHOW PROCEDURE STATUS WHERE Db = 'IonSF';
DROP EVENT IonSF.UpdateMarchMadnessR3Table;
CREATE EVENT IonSF.UpdateMarchMadnessR3Table
ON SCHEDULE EVERY 1 HOUR
STARTS CURRENT_TIMESTAMP
DO CALL IonSF.UpdateMarchMadnessR3Table();
SELECT * FROM information_schema.events;
CALL IonSF.UpdateMarchMadnessR3Table();
DELIMITER //
CREATE PROCEDURE IonSF.UpdateMarchMadnessR3Table()
BEGIN
	-- Drop the new table if it exists from a previous incomplete run
    DROP TABLE IF EXISTS IonSF.MarchMadnessR3Table_New;
    -- Create the new table with a temporary name
    CREATE TABLE IonSF.MarchMadnessR3Table_New AS
    WITH R2Winner AS (
		SELECT *
	...
	R3Winner AS (
		SELECT 
			...
			Region_A,
			Seed_A,
			a.R3_Total_Points AS R3_Total_Points_A,
			Winner_B,
			Region_B,
			Seed_B,
    ...
    RENAME TABLE IonSF.MarchMadnessR3Table TO IonSF.MarchMadnessR3Table_Old;

    -- Rename the new table to the original table's name
    RENAME TABLE IonSF.MarchMadnessR3Table_New TO IonSF.MarchMadnessR3Table;

    -- Drop the old table
    DROP TABLE IF EXISTS IonSF.MarchMadnessR3Table_Old;
