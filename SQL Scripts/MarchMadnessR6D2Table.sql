DROP PROCEDURE IF EXISTS IonSF.UpdateMarchMadnessR6D2Table;
SHOW PROCEDURE STATUS WHERE Db = 'IonSF';
DROP EVENT IonSF.UpdateMarchMadnessR6D2Table;
CREATE EVENT IonSF.UpdateMarchMadnessR6D2Table
ON SCHEDULE EVERY 1 HOUR
STARTS '2024-03-21 00:00:00'
DO CALL IonSF.UpdateMarchMadnessR6D2Table();
SELECT * FROM information_schema.events;
CALL IonSF.UpdateMarchMadnessR6D2Table();
DELIMITER //
CREATE PROCEDURE IonSF.UpdateMarchMadnessR6D2Table()
BEGIN
	-- Drop the new table if it exists from a previous incomplete run
    DROP TABLE IF EXISTS IonSF.MarchMadnessR6D2Table_New;
    -- Create the new table with a temporary name
    CREATE TABLE IonSF.MarchMadnessR6D2Table_New AS
    WITH R5Winner AS (
		SELECT *
    ...
    RENAME TABLE IonSF.MarchMadnessR6D2Table TO IonSF.MarchMadnessR6D2Table_Old;

    -- Rename the new table to the original table's name
    RENAME TABLE IonSF.MarchMadnessR6D2Table_New TO IonSF.MarchMadnessR6D2Table;

    -- Drop the old table
    DROP TABLE IF EXISTS IonSF.MarchMadnessR6D2Table_Old;
